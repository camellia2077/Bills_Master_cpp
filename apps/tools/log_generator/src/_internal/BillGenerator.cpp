...
auto BillGenerator::load_config(const std::string& config_path) -> bool {
  std::ifstream config_file(config_path);
  if (!config_file.is_open()) {
    std::cerr << "Error: Could not find or open configuration file '"
  ...
  try {
    ...
    if (raw_config.is_array()) {
      ...
      std::cerr
          << "Error: Config root must be an array of sub-category objects."
          << std::endl;
      return false;
    }
    ...
    if (raw_config.contains("comment_options")) {
      ...
      if (comment_opts.contains("comments") &&
          comment_opts["comments"].is_array()) {
