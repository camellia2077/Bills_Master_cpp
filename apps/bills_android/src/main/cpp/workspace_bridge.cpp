...
auto import_sample(const std::string& input_path, const std::string& config_dir,
  ...
  const auto result = BillWorkflowService::Ingest(
      *source_documents, validated.runtime_config, *repository, false);
  ...
  if (!result.files.empty()) {
    ...
    if (first_failure != result.files.end()) {
      ...
      if (!detail_summary.empty()) {
        message = result.success == 0U ? detail_summary
...
Java_com_billstracer_android_data_nativebridge_WorkspaceNativeBindings_importBundledSampleNative(
