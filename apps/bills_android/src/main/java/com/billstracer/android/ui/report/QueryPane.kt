...
import com.billstracer.android.ui.common.YearDigitsField
...
import com.billstracer.android.ui.common.yearInputValidationMessage

@Composable
internal fun QueryPane(
    state: BillsUiState,
    ...
    val monthQueryError = manualRecordPeriodValidationMessage(
        yearInput = state.queryPeriodYearInput,
        monthInput = state.queryPeriodMonthInput,
    )
    val isMonthQueryValid = manualRecordPeriodOrNull(
        yearInput = state.queryPeriodYearInput,
        monthInput = state.queryPeriodMonthInput,
    ) != null
    ...
    PaneContent(modifier = modifier) {
        ...
        state.queryResult?.let { result ->
            queryResultDisplayContent(result = result)
        }
    }
}
