// serialization/BillJsonSerializer.cpp

#include "BillJsonSerializer.hpp"
...
auto format_money(double value) -> nlohmann::json {
    std::stringstream ss;
    ss << std::fixed << std::setprecision(kMoneyPrecision) << value;
    return nlohmann::json::parse(ss.str());
}
}  // namespace

auto BillJsonSerializer::read_from_file(const std::string& file_path) -> ParsedBill {
...
void BillJsonSerializer::write_to_file(const ParsedBill& bill_data,
    ...
    if (!output_file.is_open()) {
        throw std::runtime_error("无法打开输出账单文件 '" + file_path + "' 进行写入。");
    }
...
auto BillJsonSerializer::deserialize(const nlohmann::json& data) -> ParsedBill {
    ...
    try {
        const std::string date_str = data.at("date").get<std::string>();
        ...
        if (date_str.length() == kDateLength) {
            bill_data.year = std::stoi(date_str.substr(0, kYearLength));
            bill_data.month = std::stoi(date_str.substr(kYearLength, kMonthLength));
        } else {
...
auto BillJsonSerializer::to_json(const ParsedBill& bill_data)
    ...
    root["total_income"] = format_money(bill_data.total_income);
    root["total_expense"] = format_money(bill_data.total_expense);
    root["balance"] = format_money(bill_data.balance);
    ...
    for (const auto& transaction : bill_data.transactions) {
        auto it = parents.find(transaction.parent_category);
        if (it == parents.end()) {
            parent_order.push_back(transaction.parent_category);
            it = parents.emplace(transaction.parent_category, ParentAggregate{}).first;
        }
        ...
        it->second.transactions.push_back(transaction_node);
        it->second.sub_total += transaction.amount;
    }
    ...
    for (const auto& parent_title : parent_order) {
        ...
        parent_node["sub_total"] = format_money(aggregate.sub_total);
