...
YearlyReportGenerator::YearlyReportGenerator(
    sqlite3* db_connection, const std::string& plugin_directory_path)
...
// 构造函数2: 从文件列表加载
YearlyReportGenerator::YearlyReportGenerator(
    sqlite3* db_connection, const std::vector<std::string>& plugin_file_paths)
    : m_reader(db_connection),
      // 在初始化列表中传入年度插件的后缀
      m_plugin_manager("_year_formatter") {
  // 在函数体中调用加载方法
...
auto YearlyReportGenerator::generate(int year, const std::string& format_name)
  ...
  if (!formatter) {
    ...
    throw std::runtime_error(
        ...
        "' is not available. Please ensure that the plugin file '" +
        expected_dll_name + "' exists in the plugins directory.");
  }
