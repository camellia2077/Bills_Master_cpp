...
#include "reports/monthly_report/ReportData.hpp"
...
class IMonthReportFormatter {
 public:
  virtual ~IMonthReportFormatter() =
      default;  // 虚析构函数，保证派生类对象能被正确销毁
   ...
   */
  virtual std::string format_report(
      const MonthlyReportData& data) const = 0;  // 纯虚函数
};
