...
void ReportExporter::save_report(const std::string& report_content,
...
auto ReportExporter::get_display_format_name(const std::string& short_name)
    -> std::string {
  if (short_name == "md") {
    return "Markdown";
  }
...
void ReportExporter::export_yearly(const std::string& report_content,
  ...
  auto format_it = m_format_folder_names.find(format_name);
  format_folder = (format_it != m_format_folder_names.end())
                      ? format_it->second
                      : get_display_format_name(format_name) + "_bills";
...
void ReportExporter::export_monthly(const std::string& report_content,
  ...
  fs::path target_dir =
      m_export_base_dir / format_folder / "months" / month_str.substr(0, 4);
