...
QueryFacade::QueryFacade(
    const std::string& db_path, const std::string& plugin_directory_path,
    const std::string& export_base_dir,
    const std::map<std::string, std::string>& format_folder_names)
    : m_db(nullptr) {
  if (sqlite3_open_v2(db_path.c_str(), &m_db, SQLITE_OPEN_READONLY, nullptr) !=
      SQLITE_OK) {
    throw std::runtime_error("Cannot open database: " +
                             std::string(sqlite3_errmsg(m_db)));
  }
  m_monthly_generator =
      std::make_unique<MonthlyReportGenerator>(m_db, plugin_directory_path);
  m_yearly_generator =
      std::make_unique<YearlyReportGenerator>(m_db, plugin_directory_path);
  m_report_exporter =
...
QueryFacade::QueryFacade(
    const std::string& db_path, const std::vector<std::string>& plugin_paths,
    const std::string& export_base_dir,
    const std::map<std::string, std::string>& format_folder_names)
    : m_db(nullptr) {
  if (sqlite3_open_v2(db_path.c_str(), &m_db, SQLITE_OPEN_READONLY, nullptr) !=
  ...
  m_monthly_generator =
      std::make_unique<MonthlyReportGenerator>(m_db, plugin_paths);
  m_yearly_generator =
      std::make_unique<YearlyReportGenerator>(m_db, plugin_paths);
  m_report_exporter =
      std::make_unique<ReportExporter>(export_base_dir, format_folder_names);
  m_metadata_reader = std::make_unique<BillMetadataReader>(m_db);
}
...
auto QueryFacade::export_yearly_report(const std::string& year_str,
...
auto QueryFacade::export_monthly_report(const std::string& month_str,
                                        const std::string& format_name,
                                        bool suppress_output) -> bool {
  try {
...
auto QueryFacade::export_by_date(const std::string& date_str,
...
auto QueryFacade::export_by_date_range(const std::string& start_date,
  ...
  try {
    std::vector<std::string> all_months =
        m_metadata_reader->get_all_bill_dates();
...
auto QueryFacade::export_all_monthly_reports(const std::string& format_name)
    -> bool {
  ProcessStats stats;
...
auto QueryFacade::export_all_yearly_reports(const std::string& format_name)
    -> bool {
  ProcessStats stats;
...
auto QueryFacade::export_all_reports(const std::string& format_name) -> bool {
