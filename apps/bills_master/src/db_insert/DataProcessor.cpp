...
#include "serialization/BillJsonSerializer.hpp"

auto DataProcessor::process_and_insert(const std::string& bill_file_path,
                                       const std::string& db_path) -> bool {
  try {
    // 步骤 1: 使用序列化器解析文件
    std::cout << "\n--- 开始解析JSON文件: " << bill_file_path << " ---\n";
    ParsedBill bill_data = BillJsonSerializer::read_from_file(bill_file_path);
    std::cout << "文件解析成功，找到 " << bill_data.transactions.size()
              << " 条交易记录。\n";
