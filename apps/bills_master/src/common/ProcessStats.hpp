...
struct ProcessStats {
   ...
   * @param process_name The name of the process being summarized (e.g.,
   * "Validation").
   */
  void print_summary(const std::string& process_name) const {
    std::cout << "\n--- " << process_name << " Summary ---\n";
    std::cout << GREEN_COLOR << "Successful operations: " << RESET_COLOR
              << success << "\n";
    std::cout << RED_COLOR << "Failed operations:     " << RESET_COLOR
              << failure << "\n";
    std::cout << "--------------------------------\n";
  }
};
