// conversion/modifier/config_loader/ConfigLoader.cpp

#include "ConfigLoader.hpp"

auto ConfigLoader::load(const nlohmann::json& config_json) -> Config {
  ...
  if (config_json.contains("display_name_maps") &&
    ...
    for (auto it = config_json["display_name_maps"].begin();
         it != config_json["display_name_maps"].end(); ++it) {
      if (it.value().is_object()) {
        std::map<std::string, std::string> lang_map;
        for (auto lang_it = it.value().begin(); lang_it != it.value().end();
             ++lang_it) {
