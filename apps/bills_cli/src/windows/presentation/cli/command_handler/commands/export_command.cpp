...
auto ExportCommand::execute(const std::vector<std::string>& args,
  ...
  if (args.empty()) {
    throw std::runtime_error(
        "Missing sub-command for 'export'. Use 'all' or 'date'.");
  }
...
auto ExportCommand::handle_export_all(AppController& controller) -> bool {
  ...
  for (const auto& current_format : formats_to_process) {
    ...
    if (m_type_filter == "month" || m_type_filter == "m") {
      ...
      throw std::runtime_error("Unknown value for --type: '" + m_type_filter +
                               "'. Use 'month'/'m' or 'year'/'y'.");
    }
...
auto ExportCommand::handle_export_date(const std::vector<std::string>& values,
                                       AppController& controller) -> bool {
  if (values.empty()) {
    throw std::runtime_error(
