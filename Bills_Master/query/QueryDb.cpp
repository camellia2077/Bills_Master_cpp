...
// UPDATED: Pass format to the yearly generator
...
// Pass format to the monthly generator
...
// --- Data Retrieval Methods (No changes) ---
...
// UPDATED: The yearly report export now accepts the format parameter
...
// Monthly report export (no changes needed here, already correct)
...
// UPDATED: "Export all" now correctly calls the updated export_yearly_report
void QueryFacade::export_all_reports(ReportFormat format) {
    ...
    try {
        ...
        for (const auto& year : unique_years) {
            ...
            export_yearly_report(year, format, true); // Pass the format
            std::cout << GREEN_COLOR << " OK\n" << RESET_COLOR;
