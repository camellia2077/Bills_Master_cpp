...
#include "query_agent.h" // *** ADDED: Include the new query agent module ***
...
// *** MODIFIED: Added new query options ***
void print_menu() {
...
int main() {
    ...
    try {
        ...
        int choice = 0;
        while (choice != 6) { // MODIFIED: Exit condition is now 6
            print_menu();
            std::cin >> choice;
            ...
            switch (choice) {
                ...
                // *** NEW: Cases for database queries ***
                case 4: {
                    std::string year;
                    ...
                    if (!year.empty()) {
                        try {
                            QueryAgent agent("bills.db");
                            agent.display_yearly_summary(year);
                ...
                case 5: {
                    ...
                    std::getline(std::cin, month);
                     if (!month.empty()) {
                        try {
                            QueryAgent agent("bills.db");
                            agent.display_monthly_details(month);
                        } catch (const std::runtime_error& e) {
                            std::cerr << "Query failed: " << e.what() << std::endl;
                        }
                    } else {
