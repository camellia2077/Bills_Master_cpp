...
std::vector<ParentItem> BillProcessor::process(const std::string& bill_content, std::vector<std::string>& out_metadata_lines) {
...
void BillProcessor::_perform_initial_modifications(std::vector<std::string>& lines) {
...
std::vector<ParentItem> BillProcessor::_parse_into_structure(const std::vector<std::string>& lines, std::vector<std::string>& metadata_lines) const {
...
void BillProcessor::_sort_bill_structure(std::vector<ParentItem>& bill_structure) const {
...
void BillProcessor::_cleanup_bill_structure(std::vector<ParentItem>& bill_structure) const {
    for (auto& parent : bill_structure) {
...
std::vector<std::string> BillProcessor::_split_string_by_lines(const std::string& str) {
...
std::string& BillProcessor::_trim(std::string& s) {
    s.erase(s.begin(), std::find_if(s.begin(), s.end(), [](unsigned char ch) { return !std::isspace(ch); }));
    s.erase(std::find_if(s.rbegin(), s.rend(), [](unsigned char ch) { return !std::isspace(ch); }).base(), s.end());
    return s;
}

bool BillProcessor::_is_metadata_line(const std::string& line) const {
...
double BillProcessor::_get_numeric_value_from_content(const std::string& content_line) {
    try {
        size_t pos;
...
bool BillProcessor::_is_parent_title(const std::string& line) {
    for (char c : line) {
