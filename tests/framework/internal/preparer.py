...
class TestPreparer:
    ...
    def prepare_runtime_env(self):
        ...
        source_notices_dir = os.path.join(config.WORKSPACE_DIR, "notices")
        dest_notices_dir = os.path.join(self.base_dir, "notices")
        if not os.path.exists(source_notices_dir):
            print(
                f"  {constants.RED}Error: notices directory not found in workspace "
                f"output: '{source_notices_dir}'.{constants.RESET}"
            )
            return False

        shutil.copytree(source_notices_dir, dest_notices_dir)
        print(f"  {constants.GREEN}Copied notices directory: notices{constants.RESET}")
