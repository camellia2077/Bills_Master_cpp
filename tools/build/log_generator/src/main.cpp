...
auto main(int argc, char* argv[]) -> int {
  // --- 1. Argument Parsing ---
  const ProgramOptions kOptions = parse_arguments(argc, argv);

  // Handle immediate actions like help, version, or errors from parsing
  if (kOptions.action == Action::SHOW_HELP) {
    show_help(argv[0]);
    return 0;
  }
  if (kOptions.action == Action::SHOW_VERSION) {
    const std::string kAppVersion = "1.3.0";
    const std::string kLastUpdate = "2025-07-28";
    show_version(kAppVersion, kLastUpdate);
    return 0;
  }
  if (kOptions.action == Action::ERROR) {
    std::cerr << "Error: " << kOptions.error_message << std::endl;
    show_help(argv[0]);
    return 1;
  }
  ...
  if (!generator.load_config(kConfigFilename)) {
  ...
  try {
    ...
    std::cerr << "Fatal: Failed to create base directory: " << e.what()
              << std::endl;
    return 1;
  }

  std::cout << "Configuration loaded. Generating bill files from "
            << kOptions.start_year << " to " << kOptions.end_year << "..."
            << std::endl;

  auto start_time = std::chrono::high_resolution_clock::now();

  for (int year = kOptions.start_year; year <= kOptions.end_year; ++year) {
    ...
    try {
      ...
      std::cerr << "Error: Failed to create directory for year " << year << ": "
                << e.what() << std::endl;
  ...
  int year_duration = kOptions.end_year - kOptions.start_year + 1;
  int total_files_generated = year_duration * 12;

  // --- 4. Final Summary ---
  std::cout << "\n----------------------------------------" << std::endl;
  std::cout << "Successfully generated files: " << total_files_generated
            << std::endl;
  std::cout << "----------------------------------------" << std::endl;
