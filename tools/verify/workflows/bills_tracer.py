...
def parse_requested_formats(forwarded: list[str]) -> list[str]:
    args, _ = parse_forwarded_args(
        forwarded,
        lambda parser: parser.add_argument("--formats", default="md,json,tex,rst,typ"),
    )
    ...
    for fmt in formats:
        if fmt not in SUPPORTED_VERIFY_FORMATS:
            raise ValueError(
                f"Unsupported format '{fmt}' for verify bills-tracer. "
...
def run_bills_tracer_workflow(
    repo_root: Path,
    python_exe: str,
    forwarded: list[str],
) -> int:
    assert_no_legacy_flags(forwarded, source="verify bills-tracer")
    ...
    return run_pipeline_steps(
        ...
        output_root=str(resolve_logic_pipeline_root(repo_root, "verify_bills_tracer")),
        steps=steps,
        run_id_prefix=output_project,
    )


def run_bills_tracer_parallel_smoke(
    repo_root: Path,
    python_exe: str,
    forwarded: list[str],
) -> int:
    ...
    assert_no_legacy_flags(passthrough, source="verify bills-tracer-parallel-smoke")
    ...
    pipeline_code = run_pipeline_steps(
        repo_root=repo_root,
        python_exe=python_exe,
        pipeline_name="verify_bills_tracer_parallel_smoke",
        pipeline_description="verify bills_tracer parallel smoke/gate via TOML pipeline",
        output_root=str(resolve_logic_pipeline_root(repo_root, "verify_bills_tracer_parallel_smoke")),
...
def run_artifact_tests(
    ...
    def configure_parser(parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            help="basic=bills-tracer, smoke=bills-tracer-parallel-smoke, gate=report-consistency-gate, full=basic+gate",
        )
        parser.add_argument(
            "--enforce-performance-gate",
            action="store_true",
    ...
    if args.scope in ("basic", "full"):
        bills_code = run_bills_tracer_workflow(
            repo_root=repo_root,
            python_exe=python_exe,
            forwarded=[],
        )
    ...
    if args.scope == "smoke":
        return run_bills_tracer_parallel_smoke(
            repo_root=repo_root,
            python_exe=python_exe,
