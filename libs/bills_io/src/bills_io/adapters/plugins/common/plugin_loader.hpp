// reports/plugins/common/PluginLoader.hpp
...
class PluginLoader {
 public:
  ...
  void loadPluginsFromDirectory(const std::string& plugin_path) {
    ...
    std::cout << "Scanning for plugins with suffix '" << m_plugin_suffix
              << "' in: " << plugin_path << std::endl;
  ...
  bool loadPlugin(const std::string& plugin_file_path) {
    ...
    if (!std::filesystem::is_regular_file(path) ||
        path.stem().string().rfind(m_plugin_suffix) == std::string::npos) {
      return false;
    }
    ...
    if (!create_func) {
      std::cerr << "Error: Could not find function '" << function_name
    ...
    std::cout << "  -> Loaded plugin '" << format_name << "' with suffix '"
              << m_plugin_suffix << "' from " << path.filename().string()
              << std::endl;
    return true;
  }
