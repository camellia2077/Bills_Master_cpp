...
auto handle_record_preview_command(const Json& request) -> std::string {
  ...
  if (input_path.empty()) {
    return core_abi::make_response(
        ...
        "record_preview requires non-empty 'params.input_path'.");
  }

  RecordTemplateResult<RecordPreviewResult> preview_result =
  ...
  if (!config_dir.empty()) {
    preview_result =
        RecordTemplateService::PreviewRecords(input_path, config_dir);
  } else {
    ...
    preview_result = RecordTemplateService::PreviewRecords(
        input_path, validator_config_path, modifier_config_path);
  }

  if (!preview_result) {
    return MakeServiceErrorResponse(preview_result.error(),
                                    "Failed to preview record files.");
  }
  ...
  for (const auto& preview_file : preview_result->files) {
    Json item;
    item["path"] = preview_file.path.string();
    item["ok"] = preview_file.ok;
    if (preview_file.ok) {
      item["period"] = preview_file.period;
      item["year"] = preview_file.year;
      item["month"] = preview_file.month;
      item["transaction_count"] = preview_file.transaction_count;
      item["total_income"] = preview_file.total_income;
      item["total_expense"] = preview_file.total_expense;
      item["balance"] = preview_file.balance;
    } else {
      item["error"] = preview_file.error;
    }
    files.push_back(std::move(item));
  }

  Json data;
  data["input_path"] = preview_result->input_path.string();
  data["processed"] = preview_result->processed;
  data["success"] = preview_result->success;
  data["failure"] = preview_result->failure;
  data["periods"] = preview_result->periods;
  data["files"] = std::move(files);

  if (preview_result->failure == 0U) {
    return core_abi::make_response(true, core_abi::error_code::kOk,
                                   "Record preview completed successfully.",
                                   std::move(data));
  }
