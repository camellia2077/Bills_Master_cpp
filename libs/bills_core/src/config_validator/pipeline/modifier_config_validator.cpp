...
auto ModifierConfigValidator::validate(const nlohmann::json& config_json,
  ...
  if (config_json.contains("auto_renewal_rules")) {
    ...
    if (renewal_config.contains("enabled") &&
        !renewal_config["enabled"].is_boolean()) {
      error_message =
          "配置错误: 'auto_renewal_rules' 对象中的 'enabled' "
          "键值必须是布尔类型。";
      return false;
    }
    if (renewal_config.contains("rules")) {
      if (!renewal_config["rules"].is_array()) {
        error_message =
            "配置错误: 'auto_renewal_rules' 对象中的 'rules' 键值必须是数组。";
        return false;
      }
      for (const auto& rule : renewal_config["rules"]) {
        ...
        if (!rule.contains("header_location") ||
            !rule["header_location"].is_string()) {
          error_message =
              "配置错误: 'rules' 对象缺少 'header_location' 字符串键。";
          return false;
        }
