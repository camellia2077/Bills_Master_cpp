...
def compile_rst_files(source_dir, font_name):
    """
    遍历指定目录下的所有 .rst 文件，使用 pandoc 和 xelatex 引擎进行编译，
    并在输出目录中保留原始的文件夹层级结构。

    :param source_dir: 包含 .rst 文件的源文件夹路径。
    :param font_name: 用于PDF正文的字体名称。
    """
    # 1. 定义基础输出文件夹路径
    base_output_dir = os.path.join(os.getcwd(), 'pdf_from_rst')

    # 将输入的源目录转换为绝对路径，以确保路径计算的准确性
    source_dir = os.path.abspath(source_dir)

    found_rst_file = False

    # 2. 遍历源文件夹及其所有子文件夹
    print(f"\n开始在 '{source_dir}' 中搜索 .rst 文件...")
    for root, _, files in os.walk(source_dir):
        for file in files:
            if file.endswith('.rst'):
                found_rst_file = True
                rst_file_path = os.path.join(root, file)
                print(f"\n--- 发现 RST 文件: {rst_file_path} ---")
    ...
    if not found_rst_file:
        print(f"在目录 '{source_dir}' 中没有找到任何 .rst 文件。")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="编译目录下的所有 .rst 文件为 PDF，可指定字体。",
        epilog="示例: python compile_rst.py /path/to/your/rst/files --font \"Noto Serif SC\""
    )
    parser.add_argument(
        "source_dir",
        type=str,
        help="包含 .rst 文件的源文件夹路径。"
    )
    parser.add_argument(
        "--font",
        type=str,
    ...
    compile_rst_files(args.source_dir, args.font)
