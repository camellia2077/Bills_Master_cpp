...
#include "reports/plugins/month_formatters/i_month_report_formatter.hpp"
...
class BaseMonthReportFormatter : public IMonthReportFormatter {
 public:
  /**
   * @brief Final implementation that provides the overall structure for
   * generating a report.
   *
   * This method handles common tasks like checking for data availability and
   * sorting. It then delegates the actual formatting of specific report
   * sections to the pure virtual functions that must be implemented by derived
   * classes.
   */
 ...
 protected:
   ...
   */
  virtual std::string get_no_data_message(
      const MonthlyReportData& data) const = 0;

  /**
   * @brief Generates the header of the report (e.g., LaTeX preamble, Typst
   * settings).
  ...
  /**
   * @brief Generates the summary section (total income, expense, balance,
   * remarks).
   * @param data The report data.
