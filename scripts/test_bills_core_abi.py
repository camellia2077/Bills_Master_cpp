...
def run_build(build_mode: str) -> None:
    command = [
        sys.executable,
        str(REPO_ROOT / "scripts" / "build_bills_core.py"),
        build_mode,
        "--shared",
    ]
...
def detect_library_path(build_dir_name: str) -> Path:
    base_dir = REPO_ROOT / "apps" / "bills_core" / build_dir_name / "bin"
...
def select_fixture_txt_path() -> Path:
    fixture_root = REPO_ROOT / "test" / "data" / "bills_output_from_config"
...
def run_fixture_command_tests(
    ...
    *,
    ...
    fixture_config_dir = REPO_ROOT / "test" / "config"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run C ABI compatibility smoke tests for apps/bills_core."
    )
    parser.add_argument(
        "--build-mode",
    ...
    parser.add_argument(
        "--build-dir",
        default="build_fast",
        help="Build directory name under apps/bills_core (default: build_fast).",
    )
    parser.add_argument(
        "--lib",
        dest="library_path",
    ...
    parser.add_argument(
        "--skip-build",
        action="store_true",
        help="Skip invoking scripts/build_bills_core.py.",
    )
    parser.add_argument(
        "--smoke-loops",
        type=int,
