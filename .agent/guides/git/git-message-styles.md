...
- 补写 `[Component Versions]` 时，优先以 `docs/release/presentation/android` 或 `docs/release/presentation/cli` 中已落盘的 history/version 口径为准；若 docs 未明确，再参考代码中的模块版本号；仍不明确时使用 `changed` / `unchanged` 之类的状态描述
